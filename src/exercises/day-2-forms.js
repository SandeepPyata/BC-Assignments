...
function SignupForm(){
  ...
  const element = (
    <form onSubmit={handleSubmit}>
      <label>Name</label>
      <input type="text" defaultValue={name} onChange={handleName} id="name" required/><br />

      <label>Email</label>
      <input type="email" defaultValue={email} onChange={handleEmail} required/><br/>
      {validEmail && <span>Incorrect Email<br/></span>}

      <label>Password</label>
      <input type="password" defaultValue={pass} onChange={handlePass}  required/><br/>

      <label>Confirm Password</label>
      <input type="password" defaultValue={confirmPass} onChange={handleConfirmPass} required/><br/>
      {matchPass && <span>Passwords didn't match.<br/></span>}
